...
blogsRouter.put('/:id/comments', async (request, response) => {
  const body = request.body
  if(!body.content) {
    return response.status(400).json('comment missing')
  }
  ...
  const blog = {
    title: commentedBlog.title,
    author: commentedBlog.author,
    url: commentedBlog.url,
    likes: commentedBlog.likes,
    comments: commentedBlog.comments.concat(body.content)
  }
...
blogsRouter.put('/:id', userExtractor, async (request, response) => {
  ...
  if(body.likes === blogToUpdate.likes && user._id.toString() !== blogToUpdate.user.toString()) {
